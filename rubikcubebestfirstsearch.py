...
class GACube:
    ...
    def Best_First_Search(self, heuristic):
        ...
        while not pq.empty():
            curr_node = pq.get()
            cube_str = str(curr_node.cube)

            if curr_node.cube == self.cube_solved:
                self.cube = curr_node.cube
            ...
            for move in valid_moves:
                temp_cube = copy.deepcopy(curr_node.cube)
                self.cube = temp_cube
                getattr(self, move)()
                new_node = NodeB(self.cube)
                new_node.path = curr_node.path + [move]
